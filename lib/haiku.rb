...
class Haiku
  DICTIONARY = {
    singular: %w(you her him sea sky ice tea air sun zen dove seed bell rock wave
    ...
    plural: %w(ova ans lips eyes jars gems eggs monks birds rocks seeds wings
    ideas faces waves bells hands monks trees clouds petals stones leaves apples
    pebbles dreams hearts petals wounds colors meadows mirrors candles flowers
    whispers shadows breaths temples gardens diamonds blankets droplets whispers
    birthdays mountains waterfalls scarecrows jellybeans),
    ...
    elegant secluded merciful nameless stunning painful soaring enormous tranquil
    marvelous beautiful startling wonderful refreshing delightful weightless
  ...
  def sentence(words)
    ...
    # best = sentences.sample
    best = sentences.sort_by(&:size).first # Pick the longest sentence.
    if best
