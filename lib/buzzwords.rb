...
class Buzzwords
  DICTIONARY = {
    ...
    verify evolve extend manage design target retain deliver embrace empower enhance
    exploit monitor develop approve rebrand utilize automate expedite generate
    incubate leverage maximize monetize optimize redefine reinvent aggregate
    architect benchmark cultivate implement integrate repurpose syndicate synergize
    transform facilitate productize streamline synthesize transition),

    plural: %w(UIs GUIs hubs slots specs users models niches stocks actions markets
    metrics portals schemas systems designs drivers channels eyeballs! networks
    services pageviews! paradigms platforms customers solutions synergies appliances
    interfaces),

    singular: %w(GUI XML hub HTML JSON core spec focus frame group model access
    engine matrix! policy portal system ability! adapter archive circuit concept!
    loyalty network pricing! process product project! support synergy toolset website
    Internet Intranet alliance analyzer approach business capacity commerce customer
    ...
    challenge! emulation framework groupware hierarchy interface knowledge migration
    moderator readiness structure taskforce timeframe workforce capability
    complexity encryption enterprise initiative management middleware monitoring
    moratorium projection throughput mindshare! console),

    adjective: %w(new hot full open meme local solid smart viral global killer
    static robust secure modern backend foreign diverse dynamic focused leading
    organic virtual advanced balanced critical enhanced expanded extended frontend
    granular european holistic magnetic optional profound reactive realtime scalable
    seamless shareable vertical wireless automated budgetary digitized downsized
    efficient ergonomic exclusive impactful intuitive mandatory networked optimized
    polarised proactive strategic universal versatile visionary worldwide artificial
    ...
    nine ten eleven twelve sixteen hundreds thirteen seventeen thousands thirteenth
    eighteenth) }
